...
def precompute_all_indicators(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    bp7 = bp.rolling(7, min_periods=1).sum()
    tr7 = tru.rolling(7, min_periods=1).sum().replace(0, np.nan)
    bp14 = bp.rolling(14, min_periods=1).sum()
    tr14 = tru.rolling(14, min_periods=1).sum().replace(0, np.nan)
    bp28 = bp.rolling(28, min_periods=1).sum()
    tr28 = tru.rolling(28, min_periods=1).sum().replace(0, np.nan)
    working["uo"] = (100 * (4 * bp7 / tr7 + 2 * bp14 / tr14 + bp28 / tr28) / 7).fillna(50.0)
    ...
    working["trade_intensity"] = (trades_arr / trades_arr.rolling(20, min_periods=3).mean().replace(0, np.nan)
                                  ).replace([np.inf, -np.inf], np.nan).fillna(1.0)
