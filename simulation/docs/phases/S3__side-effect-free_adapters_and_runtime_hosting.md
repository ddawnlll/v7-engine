# S3 — Side-Effect-Free Adapters & Runtime Hosting
...
**Phase:** `S3`
**One-line goal:** Expose simulation safely to V7 runtime and AlphaForge without hidden simulators.
**Why now:** Simulation must be consumable by V7 (paper/replay/live outcome normalization) and AlphaForge (labels/evaluation) through deterministic, side-effect-free adapters.
...
|---|---|
| Phase | `S3` |
| Title | `Side-Effect-Free Adapters & Runtime Hosting` |
| Status | `Planned` |
| Last updated | `2026-06-01` |
| Delivery status | `Not started` |
| Target environment | `Local / Staging` |
| Primary focus | `Expose simulation safely to V7 runtime and AlphaForge without hidden simulators.` |
...
Simulation must be consumable by V7 (paper/replay/live outcome normalization) and AlphaForge (labels/evaluation) through deterministic, side-effect-free adapters.

Training/evaluation must not have live side effects. Paper/replay must use identical semantics. AlphaForge must not implement a hidden simulator. V7 runtime hosts simulation operationally through stable contracts.
...
* Training replay adapter = not implemented.
* Evaluation replay adapter = not implemented.
* Historical replay driver = not implemented.
* Paper forward driver = not implemented.
* V7 runtime host integration = not defined.
* AlphaForge label adapter contract = not defined.
...
### S3.A — Training Replay Adapter

**Goal:** Constructs SimulationInput from canonical historical state and future windows.
...
**Requirements:**
* Constructs SimulationInput from canonical historical state and future windows.
* No live execution side effects.
* Emits versioned SimulationOutput with TRAINING adapter_kind.
...
* Worktree isolation required.
### S3.B — Evaluation Replay Adapter

**Goal:** Produces identical output to training adapter for identical input.
...
**Requirements:**
* Produces identical output to training adapter for identical input.
* No live execution side effects.
* Preserves evaluation_run_id lineage.
...
* Expected batch: batch_1
* Worktree isolation required.
### S3.C — Historical Replay Driver

**Goal:** Drives replay over historical data using same engine.
...
**Requirements:**
* Drives replay over historical data using same engine.
* Must not call live exchange, broker, or mutable account-state paths.
* Uses replay-safe inputs only.
...
**Isolation & Parallelism Notes:**
* Depends on S3.A for foundation.
* Expected batch: batch_2
* Worktree isolation required.
### S3.D — Paper Forward Driver

**Goal:** Projects forward outcomes for paper trading.
...
**Requirements:**
* Projects forward outcomes for paper trading.
* Same simulation engine semantics as replay/training.
* No live order submission or broker interaction.
...
* Expected batch: batch_2
* Worktree isolation required.
### S3.E — V7 Runtime Host Integration

**Goal:** V7 runtime host contract defined: import /simulation, route inputs/outputs.

**Dependencies:** S3.C, S3.D
...
**Requirements:**
* V7 runtime host contract defined: import /simulation, route inputs/outputs.
* V7 runtime does not modify simulation engine logic.
* Regime constraints recorded separately from simulation outputs.
...
**Isolation & Parallelism Notes:**
* Depends on S3.C, S3.D for foundation.
* Expected batch: batch_3
* Worktree isolation required.
### S3.F — AlphaForge Label Adapter Contract

**Goal:** AlphaForge P2 updated to consume /simulation adapter outputs.

**Dependencies:** S3.A, S3.B
**Parallel Group:** batch_3
**Risk Level:** high
**Queue Priority:** critical
**Can run with:** None
...
**Requirements:**
* AlphaForge P2 updated to consume /simulation adapter outputs.
* Label builder maps SimulationOutput fields to alphaforge label schema.
...
**Isolation & Parallelism Notes:**
* Depends on S3.A, S3.B for foundation.
* Expected batch: batch_3
* Worktree isolation required.
...
```text
  Batch batch_1: S3.A + S3.B
  Batch batch_2: S3.C + S3.D
  Batch batch_3: S3.E + S3.F
```
...
`S3` is complete when ALL are true:
...
`S4` inherits: Execution contract with worktree mode, scale-mode validation rules, integration queue requirements, workspace metadata, and review hardening invariants.
...
Implement `S3` — Expose simulation safely to V7 runtime and AlphaForge without hidden simulators.

The agent must optimize for safe parallelism, not maximum concurrency.

## Hard Requirements

1. All S3 workstreams must pass acceptance criteria.
...
```json
{
  ...
  "workspaces": [
    ...
    {
      "id": "S3.B",
      "title": "Evaluation Replay Adapter",
      "dependencies": [],
      "parallelGroup": "batch_1",
      "dependencyReason": "Foundation workspace for this phase.",
      "manualApplicationRequired": false,
    ...
    {
      "id": "S3.C",
      "title": "Historical Replay Driver",
      "dependencies": [
        "S3.A"
      ],
      "parallelGroup": "batch_2",
      "dependencyReason": "Depends on S3.A for foundation.",
      "manualApplicationRequired": false,
      "humanApprovalRequired": false,
      "autonomousExecutionAllowed": true,
      "rollbackRequired": true,
    ...
    {
      "id": "S3.E",
      "title": "V7 Runtime Host Integration",
      "dependencies": [
        "S3.C",
        "S3.D"
      ],
      "parallelGroup": "batch_3",
      "dependencyReason": "Depends on S3.C, S3.D for foundation.",
      "manualApplicationRequired": false,
      "humanApprovalRequired": false,
      "autonomousExecutionAllowed": true,
      "rollbackRequired": true,
    ...
    {
      "id": "S3.F",
      "title": "AlphaForge Label Adapter Contract",
      "dependencies": [
        "S3.A",
        "S3.B"
      ],
      "parallelGroup": "batch_3",
      "dependencyReason": "Depends on S3.A, S3.B for foundation.",
      "manualApplicationRequired": false,
      "humanApprovalRequired": false,
      "autonomousExecutionAllowed": true,
      "rollbackRequired": true,
...
```json
{
  ...
  "primaryGoal": "Expose simulation safely to V7 runtime and AlphaForge without hidden simulators.",
  ...
  "completionGate": "S3 complete only when all workspaces pass acceptance criteria and final validation passes.",
  "nextPhase": "S4"
}
```
