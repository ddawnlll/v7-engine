...
for sym_idx, sym in enumerate(symbols_list):
    ...
    # FeatureMatrix has .features dict, not .matrix
    feature_keys = list(feat_matrix.features.keys())
    X_base = np.column_stack([feat_matrix.features[k] for k in feature_keys])
    ...
    all_X.append(X_aug); all_y.append(y); all_r.append(r)
    all_ts.append(np.arange(n_bars, dtype=np.int64))
