...
SCHEMA_PATH = ROOT / "schemas" / "alphaforge" / "mode_research_report.schema.json"
FIXTURE_PATH = ROOT / "fixtures" / "alphaforge" / "swing_mode_research_report_minimal.json"
...
class TestSchemaRejectsMissingActiveMetrics:
    ...
    def test_missing_active_trade_count_is_rejected(self):
        ...
        schema = load_schema()
        # Check if active_trade_count is required at the schema level
        metrics_required = (
...
class TestMultipleHypothesisControlDeduplication:
    ...
    def test_raw_schema_file_has_single_multiple_hypothesis_control_property(self):
        ...
        # Find all occurrences of "multiple_hypothesis_control" that are property key definitions
        # (followed by ": {" — property definition, not in required array)
        property_defs = re.findall(
            r'"multiple_hypothesis_control"\s*:\s*\{',
            raw,
        )
        # A properly deduplicated schema should have exactly 1 property definition
        # (the required array reference at top level is OK)
    ...
    def test_mht_required_fields_include_mht_status(self):
        """After dedup, multiple_hypothesis_control required must include mht_status."""
        schema = load_schema()
        mht_prop = schema.get("properties", {}).get("multiple_hypothesis_control", {})
        mht_required = mht_prop.get("required", [])
        # The second (surviving) definition required: tested_hypothesis_count, correction_method, data_snooping_risk_flag
        # We expect mht_status to also be required after dedup/fix
